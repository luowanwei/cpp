...
{
	typename Container::const_iterator cit = c.begin();//获取到容器的迭代器
	for(; cit != c.end(); ++cit) {
		cout << *cit << " ";
	}
	cout << endl;
...
void test0()
{
	...
	display(numbers);
	numbers.push_back(6);// O(1)，在尾部添加元素
	display(numbers);
	...
	vector<int>::iterator it = numbers.begin();//获取到开始迭代器的位置
	cout << "*it = " << *it << endl;
...
void test1()
{
	...
	deque<int> numbers{1, 2, 3, 4, 5};//同上两行写法一致
	display(numbers);
	numbers.push_back(6);//O(1)
	display(numbers);
	numbers.push_front(0);//O(1)，在头部添加元素，而vactor没有这个头部操作
	display(numbers);
	//deque支持下标访问运算符 ==> 随机访问
	cout << "numbers[2] = " << numbers[2] << endl;
	//添加元素
