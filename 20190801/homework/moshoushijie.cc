...
public:
    ...
    Ninja& makeNinja()
    {
        nStroop.push_back(Ninja(_nStrength,++_n));
        _M-=_nStrength;
        return nStroop.back();
    }
     Iceman& makeIceman()
     {
         iStroop.push_back(Iceman(_iStrength, ++_n)); 
         _M = _iStrength; 
         return iStroop.back();
    } 
    Lion& makeLion()
    {
        lStroop.push_back(Lion(_lStrength, ++_n, _M));
         _M -= _lStrength; 
         return lStroop.back();
    } 
    Wolf& makeWolf()
    {
        wStroop.push_back(Wolf(_wStrength,++_n));
        _M -=_wStrength;
        return wStroop.back();
    }
...
void rollAll(center red,center blue)
{
    ...
    while(1)
    {
        ...
        if(bOK&&pBBit==0)
        {
            printf("%03d %s headerquarter stops making warriors\n",t,"blue");
            pBBit=1;
        }
        if(bOK&&rOK)
            break;
